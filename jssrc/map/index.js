...
require(['components/map/area', 'components/map/line', 'components/map/pre'], function(areaFn, lineFn) {
    function IndexController() {
        ...
        this.$customSelectLis = $('.main>.Select'); //售价，户型，面积，特色自定义筛选Select 集合
    ...
    IndexController.prototype.init = function(zoomLv, lon, lat) {
        ...
        classSelf.request(classSelf.apiUrl.houseMap.querySellListOnMap, requestData, {
            process: function(resp) {
                ...
                //城市ID与当前城市id不同，则需要重新请求新城市的区域数据和地铁数据，并重置左侧条件
                if (cityId && cityId != classSelf.$frm.attr("data-id")) {
                    ...
                    classSelf.$area.rest();
                    classSelf.$line.rest();
                    classSelf.$customSelectLis.rest();
                    classSelf.$list.rest();
                }
                ...
                requestData.pId = classSelf.$area.find(".Selected").attr('data-id');
    ...
    IndexController.prototype.setList = function(type, id, sub) {
        ...
        if (id && type) {
            ...
            classSelf.$customSelectLis.hide();
            classSelf.$list.html('').hide();
            classSelf.setHeight();
            return;
        }

        requestData.cityId = classSelf.$frm.attr('data-id');
        requestData.houseType = classSelf.houseType;
    ...
    /*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    初始化页面Select 组件的相关事件
    1. 页面所有Select元素Hover 事件，悬停展开下拉选项
    2. 自定义筛选条件事件绑定
    3. 区域找房事件绑定
    4. 地铁找房事件绑定
    5. 筛选条件重置方法
    6. 房源列表的重置方法定义
    --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    IndexController.prototype.addListenerToSelects = function() {
        var classSelf = this;

        //1.页面所有Select元素Hover 事件，悬停展开下拉选项
        $(".Select").hover(function() {
            /* Stuff to do when the mouse enters the element */
            var $dn = $(this).contents('.Dn');
        ...
        //2. 自定义筛选条件事件绑定
        classSelf.$customSelectLis.on("click", function() {
        ...
        //4. 地铁找房相关事件绑定
        classSelf.$line.find('.Selected').click(function() {
            var _this = $(this);
            var id = _this.attr('data-id');
            var lv = _this.attr('data-lv');
            ...
            //判断是否存在筛选条件
            if (id) {
                if (lv && sid) { //同时选择了地铁线路和地铁站
                    //渲染房源列表
                    classSelf.setList(4 /*按地铁站*/ , id, sid);
        ...
        classSelf.$customSelectLis.rest = function() {
            classSelf.$customSelectLis.find('.Selected').each(function(index, el) {
            ...
            classSelf.$customSelectLis.find('.act').removeClass('act');

            classSelf.$customSelectLis.hide();
        }

        //6. 房源列表重置方法定义
        classSelf.$list.rest = function() {
            classSelf.$list.removeAttr('style').removeAttr('data-id')
                .removeAttr('data-type').removeAttr('data-order');

            classSelf.$list.empty();

            classSelf.$order.remove(); //清空排序容器内容      
        }
    }
            ...
            process: function(resp) {
                ...
                for (var p, i = subway_data.stationList.length - 1; i >= 0; i--) {
                    ...
                    p.addEventListener('click', function() {
                        ...
                        //重新渲染地图
                        classSelf.init(16 /*按地铁站*/ , lon, lat);
                    });

                    //如已经选择了地铁站，设置改地铁站高亮显示，并绘制改地铁站周边的圈
    ...
    IndexController.prototype.run = function() {
        ...
        //页面所有select 组件添加事件绑定
        classSelf.addListenerToSelects();
    }
