...
require(['components/map/area', 'components/map/line'], function(areaFn, lineFn) {
    ...
    IndexController.prototype.init = function(lv, lat, lon) {
        ...
        if (lv == classSelf.data.level && bs.minLon > classSelf.data.minLon && classSelf.data.maxLon > bs.maxLon && bs.minLat > classSelf.data.minLat &&
            classSelf.data.maxLat > bs.maxLat) {
        ...
        //请求ajax 获取打点需要基础数据
        classSelf.request(classSelf.apiUrl.houseMap.querySellListOnMap, $.extend(requestData, classSelf.genSelect()), {
    ...
    IndexController.prototype.clear = function() {
        ...
        var bs = classSelf.bounds(1); //1屏清除点
    ...
    IndexController.prototype.getLevel = function(lv) {
        ...
        if (lv < classSelf.level[1]) {
            return 0;
        } else if (lv < classSelf.level[2] - 1) {
            return 1;
        } else {
            return 2;
        }
    ...
    IndexController.prototype.setCircle = function(lon, lat) {
        ...
        classSelf.map.removeOverlay(classSelf.subway.circle);
        classSelf.map.removeOverlay(classSelf.subway.remark);
        classSelf.subway.circle = new BMap.Circle(new BMap.Point(lon, lat), 1000, {
            strokeColor: '#ffa200',
            strokeWeight: 2,
            strokeOpacity: 1,
            fillColor: '#ffa200',
